...
class DealByCat_ID_Adapter(
    private val context: DealByCat_IDActivity,
    private val modelList: List<ExpiredSoonModel?>,
        ...
        try {
            if (holder is ViewHolder) {
                ...
                val productDiscountPercentageEnabled =
                    model.productDiscountPercentageEnabled.toInt()
                if (productDiscountPercentageEnabled == 0 && priceEnabledId == 0 && discountPriceEnabledId == 0) {
                    ...
                    holder.binding.tvDiscount.visibility = View.GONE
                    ...
                    holder.binding.tvDiscount.visibility = View.VISIBLE
                    holder.binding.tvFinalPrice.text =
                        model.productCurrency + model.productFinalPrice
                ...
                if (!model.productAgentImage.equals("https://www.dealdio.com/upload/empty_deal.jpg",
                        ignoreCase = true)
                ) {
                    try {
                        val transformation = RoundedTransformationBuilder()
                ...
                holder.binding.tvLeftDays.text = "Expire on " + DateConversion.local(model.dealExpiredDate)
                ...
                holder.binding.rootLayout.setOnClickListener {
                    context.moveToDetails(model.productId,
                        model.agentId,
                        position)
                }
        ...
        if (AppUtil.isNetworkAvailable(context)) {
            ...
            call!!.enqueue(object : Callback<ResponseBody?> {
                        ...
                        try {
                            ...
                            if (errorType == KeyConstant.KEY_RESPONSE_CODE_200) {
                                ...
                                context.runOnUiThread {
                                    ...
                                    if (modelList[posi]!!.productFavourite == "0") {
                                        modelList[posi]!!.productFavourite = "1"
                                        holder.binding.ivNonFavoriteIcon.visibility = View.GONE
                                        holder.binding.ivFavoriteIcon.visibility = View.VISIBLE
                                    } else {
                                        modelList[posi]!!.productFavourite = "0"
                                        holder.binding.ivNonFavoriteIcon.visibility = View.VISIBLE
                                        holder.binding.ivFavoriteIcon.visibility = View.GONE
                                    }
                                }
                                ...
                                if (KeyConstant.KEY_MESSAGE_FALSE.equals(resp.optString(KeyConstant.KEY_STATUS),
                                    ...
                                    dialogManager.stopProcessDialog()
                                    AppUtil.showMsgAlert(holder.binding.tvDiscount,
                ...
                override fun onFailure(call: Call<ResponseBody?>, t: Throwable) {
                    ...
                    dialogManager.stopProcessDialog()
                    AppUtil.showMsgAlert(holder.binding.tvDiscount, t.message)
                }
            })
        } else {
            AppUtil.showMsgAlert(holder.binding.tvDiscount,
