...
class LatestDeals : BaseActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this@LatestDeals, R.layout.dailydeals)
        setToolbarWithBackButton_colorprimary("Latest Deals")
        init()
        binding.rvDeals.setHasFixedSize(false)
        binding.rvDeals.layoutManager = WrapContentLinearLayoutManager(this@LatestDeals)
        adap = AdapterDeals(this@LatestDeals, mList, binding.rvDeals, "")
        ...
        binding.llSearch.setOnClickListener {
            val intent = Intent(this@LatestDeals, SearchAllNewActivity::class.java)
    ...
    private fun getAllDetails(isShowingLoader: Boolean) {
        ...
        if (AppUtil.isNetworkAvailable(this@LatestDeals)) {

            if (isShowingLoader) {
                if (binding.shimmerViewContainer.visibility === View.GONE) {
                    dialogManager.showProcessDialog(this@LatestDeals, "", false, null)
                }}
            else{
                if (binding.shimmerViewContainer.visibility === View.GONE) {
                    dialogManager.showProcessDialog(this@LatestDeals, "", false, null)
            ...
            val call = AppConfig.api_Interface().getAllDealsV1(agentIds, lat, lang, offset.toString(), "10", catId, "2")
                    ...
                    if (response.isSuccessful) {
                            ...
                            try {
                                ...
                                try{
                                    ...
                                    binding.mainLayout.visibility = View.VISIBLE}catch (e:Exception){}
                                ...
                                if (errorType.equals(KeyConstant.KEY_RESPONSE_CODE_200)) {
                                    ...
                                    try {
                                        if (arrDealsList.length() > 0) {
                                            ...
                                            for (i in 0 until arrDealsList.length()) {
                                                ...
                                                var productDistance = obj.optString("productDistance")
                                                var productAgentId = obj.optString("productAgentId")
                                                var productAgentName = obj.optString("productAgentName")
                                                var productAgentImage = obj.optString("productAgentImage")
                                                var productLocation = obj.optString("productLocation")
                                                var productFavourite = obj.optString("productFavourite")
                                                var productImage = obj.optString("productImage")
                                                var productCurrency = obj.optString("productCurrency")
                                                var productDiscountPercentageEnabled = obj.optString("productDiscountPercentageEnabled")
                                                var productDiscountPercentage = obj.optString("productDiscountPercentage")
                                                var productPrice = obj.optString("productPrice")
                                                var productFinalPrice = obj.optString("productFinalPrice")
                                                var priceEnabledId = obj.optString("priceEnabledId")
                                                var priceEnabledStatus = obj.optString("priceEnabledStatus")
                                                var discountPriceEnabledId = obj.optString("discountPriceEnabledId")
                                                var discountPriceEnabledStatus = obj.optString("discountPriceEnabledStatus")
                                                var offerType = obj.optString("offerType")
                                                var offerTypeId = obj.optString("offerTypeId")
                                                var dealExpiredDate = obj.optString("dealExpiredDate")
                                                var dealExclusiveStatus = obj.optString("dealExclusiveStatus")
                                                var productDDLoyaltyPrice = obj.optString("productDDLoyaltyPrice")

                                                var dl = DealListModel(productId, productName, productDistance, productAgentId, productAgentName, productAgentImage, productLocation, productFavourite, productImage, productCurrency, productDiscountPercentageEnabled, productDiscountPercentage, productPrice, productFinalPrice, priceEnabledId, priceEnabledStatus, discountPriceEnabledId, discountPriceEnabledStatus, offerType, offerTypeId, dealExpiredDate, dealExclusiveStatus, productDDLoyaltyPrice)
                                                mList.add(dl)
                                    ...
                                    if (KeyConstant.KEY_MESSAGE_FALSE.equals(resp.optString(KeyConstant.KEY_STATUS), ignoreCase = true)) {
                                        dialogManager.stopProcessDialog()
                                        AppUtil.showMsgAlert(binding.mainLayout, resp.optString(KeyConstant.KEY_MESSAGE))
                                    }
                                }
    ...
    override fun onResume() {
        ...
        StatusBarcolor.setStatusbarColor(this@LatestDeals, "")
    ...
    private fun getLocation(): Boolean {
        ...
        GPSTracker.requestSingleUpdate(this@LatestDeals, object : GPSTracker.LocationCallback {
            ...
            PreferenceHelper.getInstance(this@LatestDeals)?.lat = userLat.toString() + ""
            PreferenceHelper.getInstance(this@LatestDeals)?.lng = userLang.toString() + ""
            getLocationAddress()
            }
        })
        return userLat !== 0.0
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            R.id.llLocation -> {
                startActivityForResult(Intent(this@LatestDeals, SelectLocationActivity::class.java), 90)
            }
        }
    ...
    fun moveToDetails(productId: String?, agentId: String?, pos: Int) {
        val intent = Intent(this@LatestDeals, LatestProductDetails::class.java)
